...
default_args = {'-n':float('inf'), '-s':10, '-l':2, '-u':float('inf'), '-d':0, '-v':0, 'database_file_name':'graphdata/graph.data'}

def usage():
    print 'Usage: python main.py [-s min_support] [-n num_graph] [-l min_num_vertices] [-u max_num_vertices] [-d] database_file_name'
...
def main():
    ...
    gs = gSpan(database_file_name = opt_dict['database_file_name'], min_support = opt_dict['-s'],
        ...
        max_ngraphs = opt_dict['-n'], is_undirected = opt_dict['-d'] == 0, verbose = opt_dict['-v'])
    gs.run()
